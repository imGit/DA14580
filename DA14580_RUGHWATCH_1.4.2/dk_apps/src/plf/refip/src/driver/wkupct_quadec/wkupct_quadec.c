...
void wkupct_enable_irq(uint32_t sel_pins, uint32_t pol_pins, uint16_t events_num, uint16_t deb_time)
{
    ...
    temp = (uint8_t)((sel_pins >> 0) & 0xFF);
    SetWord16(WKUP_SELECT_P0_REG + (2 * 0), temp);
    temp = (uint8_t)((pol_pins >> 0) & 0xFF);
    SetWord16(WKUP_POL_P0_REG + (2 * 0), temp);
	
    temp = (uint8_t)((sel_pins >> 8) & 0x3F);
    SetWord16(WKUP_SELECT_P0_REG + (2 * 1), temp);
    temp = (uint8_t)((pol_pins >> 8) & 0x3F);
    SetWord16(WKUP_POL_P0_REG + (2 * 1), temp);

    temp = (uint8_t)((sel_pins >> 14) & 0x3FF);
    SetWord16(WKUP_SELECT_P0_REG + (2 * 2), temp);
    temp = (uint8_t)((pol_pins >> 14) & 0x3FF);
    SetWord16(WKUP_POL_P0_REG + (2 * 2), temp);

    temp = (uint8_t)((sel_pins >> 24) & 0xFF);
    SetWord16(WKUP_SELECT_P0_REG + (2 * 3), temp);
    temp = (uint8_t)((pol_pins >> 24) & 0xFF);
    SetWord16(WKUP_POL_P0_REG + (2 * 3), temp);
