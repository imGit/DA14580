                                       ...
                                       ke_task_id_t const src_id)
{
     ...
     *---------------------------------------------------*/
        ...
        status = attmdb_add_service(&(sample128_env.sample128_shdl), TASK_SAMPLE128,
                                             nb_att_16, nb_att_32, nb_att_128, 60); //total attributte size = 36, 16 (svc)  + 19 (desc_char) + 1 (att)
/*
										//Add Service Into Database
						attm_svc_create_db(&sample128_env.sample128_shdl, (uint8_t *)&cfg_flag,  SAMPLE128_1_IDX_NB, NULL,
                               dest_id, &sample128_att_db[0]);
						   //Disable GLS
                                    ...
                                    ke_task_id_t const src_id)
{
    ...
    if(1)//(param->conhdl == gapc_get_conhdl(sample128_env.con_info.conidx))
    {
        // Update value in DB
        attmdb_att_set_value(sample128_env.sample128_shdl + SAMPLE128_2_IDX_VAL,
                                      ...
                                      ke_task_id_t const src_id)
{
    ...
    if (KE_IDX_GET(src_id) == sample128_env.con_info.conidx)
    {
        ...
        if (char_code == SAMPLE128_1_CHAR)
        {
            ...
            if(param->last)
            {
                sample128_send_val(param->value[0],param->value[1]);//gsx add param->value[1]
            }
...
static const struct attm_desc sample128_att_db[SAMPLE128_1_IDX_NB] = 
{
	  [SAMPLE128_1_IDX_SVC]   =   /* Accelerometer service */
                               {ATT_DECL_PRIMARY_SERVICE, PERM(RD, ENABLE),
                                       sizeof(sample128_svc_gsx), sizeof(sample128_svc_gsx),
                                       (uint8_t*) &sample128_svc_gsx},
															 
    [SAMPLE128_1_IDX_CHAR]  = /* Accelerometer enable characteristic */
                               {ATT_DECL_CHARACTERISTIC, PERM(RD, ENABLE),
                                       sizeof(sample128_val1_char), sizeof(sample128_val1_char),
                                       (uint8_t*) &sample128_val1_char},
    [SAMPLE128_1_IDX_VAL] =   /* Accelerometer Enable Value */
                               {0xFFA1,  (PERM(RD, ENABLE) | PERM(WR, ENABLE)),
                                       sizeof(uint8_t), 0, (uint8_t*) NULL},
    [SAMPLE128_1_IDX_DESC] =  /* Accelerometer Enable User description */
                               {ATT_DESC_CHAR_USER_DESCRIPTION,  PERM(RD, ENABLE),
                                       SAMPLE128_1_DESC_LEN, SAMPLE128_1_DESC_LEN,
                                       (uint8_t*) sample128_1_desc},
	
    [SAMPLE128_2_IDX_CHAR] = /* Accelerometer X-Axis characteristic */
                               {ATT_DECL_CHARACTERISTIC, PERM(RD, ENABLE),
                                       sizeof(sample128_val2_char), sizeof(sample128_val2_char),
                                       (uint8_t*) &sample128_val2_char},
    [SAMPLE128_2_IDX_VAL] =   /* Accelerometer X-Axis Value */
                               {0xFFA2, PERM(RD, ENABLE) | PERM(WR, ENABLE) | PERM(NTF, ENABLE),
                                       sizeof(uint8_t), 0, (uint8_t*) NULL},
    [SAMPLE128_2_IDX_DESC] =  /* Accelerometer Enable User description */
                               {ATT_DESC_CHAR_USER_DESCRIPTION,  PERM(RD, ENABLE),
                                       SAMPLE128_2_DESC_LEN, SAMPLE128_2_DESC_LEN,
                                       (uint8_t*) sample128_2_desc},
