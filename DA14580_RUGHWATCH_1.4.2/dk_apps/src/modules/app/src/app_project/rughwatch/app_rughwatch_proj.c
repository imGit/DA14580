...
uint8_t connect_state = 2;//连接状态,初始值为断开
uint8_t connected_once	=	0;//一旦连接后变为1,连接成功之后变为0
uint8_t disconnected_once	=	0;//一旦连接后变为1,连接中断之后变为0
/*
...
//		check_state=0;//自检状态关闭
////////////////////		
//		app_timer_set(APP_BATT_ALERT_TIMER, TASK_APP, 1000);//开启定时器APP_BATT_ALERT_TIMER
...
void app_adv_func(struct gapm_start_advertise_cmd *cmd)
{
    ...
    if (device_name_avail_space > 0)
    {
        ...
        if(device_name_length > 0)
        {
			...
			//该函数被注释掉后不会进入休眠
//    app_timer_set(APP_ADV_TIMER, TASK_APP, 4000); //180000 ms -> 3 mins   randy  20140506 //断开连接后与进入休眠的间隔
...
#if BLE_BATT_SERVER
	app_batt_poll_stop(); //断开连接后停止ADC的轮询
#endif // BLE_BATT_SERVER
...
//	if(GPIO_GetPinStatus(GPIO_ALERT_CHRG_PORT,GPIO_ALERT_CHRG_PIN)==true) //处于充电状态时不进入睡眠
	{
		app_adv_stop();
		ke_timer_clear(APP_ADV_TIMER, TASK_APP);
//    TurnOff_All_Led();//gsx
...
//	else
	{
		app_timer_set(APP_ADV_TIMER, TASK_APP, 3000);//延迟30秒执行
	}
